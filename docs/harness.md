...
renders every `///|` item separator, including inline boundary documentation,
separately anchored documentation, and a documented block with no following
top-level declaration, while the underlying model retains the raw `///|` and
`///` source.

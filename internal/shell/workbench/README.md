...
- `start_app` creates and retains concrete language, marker, feedback,
  ...
  calls `mount_app`. The MoonBit adapter treats `///|`, optionally followed by
  an ASCII space and inline documentation, as an item anchor and always renders
  it as a horizontal separator. Inline text and immediately following `///`
